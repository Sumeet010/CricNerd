...
export default function Scorecard() {
  ...
  const [activeTab, setActiveTab] = useState<Tab>("Live Score");
  ...
  const loadMatchData = async () => {
    ...
    try {
      ...
      setTournament(tourRes.singleTournament);
  ...
  const handleRecordBall = async () => {
    ...
    try {
      ...
      // Reload match details and stats
      await loadMatchData();
    } catch (err: any) {
      console.error(err);
      ...
      <div className="space-y-6 pb-12">
        ...
        <div className="bg-[#121214] border border-zinc-800/80 rounded-2xl p-6 relative overflow-hidden">
          <div className="grid grid-cols-1 md:grid-cols-3 items-center gap-6">
            ...
            <div
              ...
              title={match.matchStatus !== "COMPLETED" ? `Switch active batting team to ${teamA?.teamName || "Team A"}` : undefined}
              className={`flex items-center gap-4 transition-all ${
                match.matchStatus !== "COMPLETED" ? "cursor-pointer hover:opacity-100" : ""
            ...
            <div
              ...
              title={match.matchStatus !== "COMPLETED" ? `Switch active batting team to ${teamB?.teamName || "Team B"}` : undefined}
        ...
        <div className="flex gap-1.5 border-b border-zinc-800/80 pb-0.5">
          {(["Live Score", "Scorecard", "Over Log"] as const).map((tab) => {
            if (tab === "Live Score" && match.matchStatus === "COMPLETED") return null;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {activeTab === "Live Score" && match.matchStatus !== "COMPLETED" && (
