...
import { tournamentService, teamService, matchService, playerService, squadService } from "@/services";
...
/* ─── tab list ─── */
const TABS = ["Overview", "Teams", "Matches", "Players", "Settings"] as const;
...
/* ─── Add Player Modal — Create-only (assign via right panel) ─── */
...
function AddPlayerModal({ teamId, teams, onSuccess, onCancel }: AddPlayerModalProps) {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      await playerService.create({ name: form.name.trim(), age: ageNum, playingRole: form.playingRole });
      // Player is created globally — user assigns via the right panel checkboxes
      onSuccess();
    } catch (err: any) {
  ...
  return (
    <form onSubmit={handleCreate} className="space-y-4">
      ...
      <div className="flex items-start gap-2 bg-zinc-800/60 border border-zinc-700/60 text-zinc-400 text-xs px-3 py-2.5 rounded-lg">
        ...
        <span>The player will be added to the global list. Use the checkboxes in the squad panel to assign them to a team.</span>
      </div>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap gap-2 justify-center">
        ...
        <button
          onClick={() => onOpenAddPlayer(activeTeam._id)}
            ...
            <div className="max-h-[460px] overflow-y-auto">
              {allPlayers.map((player) => {
                ...
                return (
                  <div
                    key={player._id}
                    onClick={() => !isInOtherTeam && !isLoading && handleToggle(player._id)}
...
export default function TournamentDetail() {
  ...
  const refreshTeams = async () => {
    ...
    try {
      ...
      setTeams((teamsRes.allTeams ?? []).filter((t) => t.tournamentId === id));
    } catch (err) {
  ...
  return (
    <Layout title={tournament.tournamentName} subtitle="TOURNAMENT">
      <div className="space-y-6">
        ...
        {/* ══════════════════════════════════════════
            HEADER CARD — same shell as tournament card
        ══════════════════════════════════════════ */}
        <div className="bg-[#1c1c1c] border border-zinc-800 rounded-xl overflow-hidden p-4">
          {/* Blue banner — identical to tournament card header */}
          ...
          {/* Tab bar — inside the card, same border-b as card uses */}
          <div className="flex gap-0 border-t border-zinc-800 -mx-4 px-4 mt-4">
            {TABS.map((tab) => (
              <button
                key={tab}
                ...
                {teams.map((t) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3 mt-1">
                      ...
                      <div className="flex items-center gap-1.5 bg-[#121212] border border-zinc-800 px-3 py-2 rounded-lg text-white text-xs shrink-0 font-medium">
                        <span>🏆</span>
                      ...
                      >
                        ...
                        <div className="flex items-center justify-between border-t border-zinc-800 pt-3">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            {m.matchStatus === "LIVE" && (
                              <button
                                onClick={() => navigate(`/live-scoring/${m._id}`)}
        ...
        {activeTab === "Settings" && (
          <div className="bg-[#1c1c1c] border border-zinc-800 rounded-xl overflow-hidden p-4">
            <div className="bg-[#0b36aa] h-12 rounded-lg flex items-center gap-2 px-4 mb-5">
              <Info className="w-4 h-4 text-white" />
